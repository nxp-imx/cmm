...
static int cmmRouteNetlinkLookupFilter(const struct sockaddr_nl *nladdr, struct nlmsghdr *nlh, void *arg)
{
	...
	/* Check if route MTU is not present than fallback to interface MTU */
	if (!route->mtu) {
		struct interface *itf;

		itf = __itf_find(route->oifindex);
		if (itf)
			route->mtu = itf->mtu;
	}
...
*
******************************************************************/
static void __cmmCtTunnelRouteUpdate(cpal_handle_t *cpal_handle, struct ctTable *ctEntry, struct RtEntry *route, int dir)
{
	struct ct_route rt;
	...
	if (dir == ORIGINATOR)
		__cmmRouteDeregister(cpal_handle, &rt, "originator tunnel");
	else
		__cmmRouteDeregister(cpal_handle, &rt, "replier tunnel");
}
...
*
******************************************************************/
static void __cmmCtRouteUpdate(cpal_handle_t *cpal_handle, struct ctTable *ctEntry, struct RtEntry *route, int dir)
{
	struct ct_route rt;
	...
	if (dir == ORIGINATOR)
	{
		__cmmRouteDeregister(cpal_handle, &rt, "originator");
		__cmmRouteDeregister(cpal_handle, &tunnel_rt, "originator tunnel");
	}
	else
	{
		__cmmRouteDeregister(cpal_handle, &rt, "replier");
		__cmmRouteDeregister(cpal_handle, &tunnel_rt, "replier tunnel");
	}
}
...
*
******************************************************************/
static void __cmmTunnelRouteUpdate(cpal_handle_t *cpal_handle, struct interface *itf, struct RtEntry *route)
{
	...
	__cmmRouteDeregister(cpal_handle, &rt, "tunnel");
}
...
*
******************************************************************/
static void __cmmSocketRouteUpdate(cpal_handle_t *cpal_handle, struct socket *s, struct RtEntry *route)
{
	...
	__cmmRouteDeregister(cpal_handle, &rt, "socket");
}
...
*
******************************************************************/
void __cmmRouteUpdate(cpal_handle_t *cpal_handle, struct RtEntry *route)
{
...
update:
	...
	while (entry != &ct_table_by_orig_route[key])
	{
		...
		if (ctEntry->orig.route == route)
			__cmmCtRouteUpdate(cpal_handle, ctEntry, route, ORIGINATOR);
	}
	...
	while (entry != &ct_table_by_rep_route[key])
	{
		...
		if (ctEntry->rep.route == route)
			__cmmCtRouteUpdate(cpal_handle, ctEntry, route, REPLIER);
	}
	...
	while (entry != &ct_table_by_orig_tunnel_route[key])
	{
		...
		if (ctEntry->orig_tunnel.route == route)
			__cmmCtTunnelRouteUpdate(cpal_handle, ctEntry, route, ORIGINATOR);
	}
	...
	while (entry != &ct_table_by_rep_tunnel_route[key])
	{
		...
		if (ctEntry->rep_tunnel.route == route)
			__cmmCtTunnelRouteUpdate(cpal_handle, ctEntry, route, REPLIER);
	}
	...
	for (i = 0; i < ITF_HASH_TABLE_SIZE; i++)
	{
		for (entry = list_first(&itf_table.hash[i]); entry != &itf_table.hash[i]; entry = list_next(entry))
		{
			...
			if (itf->rt.route == route)
				__cmmTunnelRouteUpdate(cpal_handle, itf, route);
		}
	}
	...
	for (i = 0; i < HASH_SOCKET_SIZE; i++)
	{
		for (entry = list_first(&socket_table[i]); entry != &socket_table[i]; entry = list_next(entry))
		{
			...
			if (s->rt.route == route)
				__cmmSocketRouteUpdate(cpal_handle, s, route);
		}
	}

	/* Check for SAs which are affected with route */
	 __cmmSAUpdateWithRoute(cpal_handle, route);
 ...
 *
 ******************************************************************/
static void __cmmRouteNew(cpal_handle_t *cpal_handle, struct rtmsg *rtm, unsigned int *dAddr, int flushed)
{
	struct list_head *entry;
	struct ctTable *ctEntry;
	struct SATable *s;
...
int cmmRtnlRoute(const struct sockaddr_nl *who, struct nlmsghdr *nlh, void *arg)
{
...
#ifdef DPDK_ENABLE
		data.r_action = ROUTE_DELETE;
#endif
	}

#ifdef DPDK_ENABLE
	data.r_depth =  rtm->rtm_dst_len;
	data.r_type = rtm->rtm_type;
	if (rtm->rtm_family == AF_INET) {
		data.msg_type = CMM_ROUTE4;
		memcpy(&data.r_nexthop, gwAddr, sizeof(struct in_addr));
		memcpy(&data.r_addr, dAddr, sizeof(struct in_addr));
	} else if (rtm->rtm_family == AF_INET6) {
		data.msg_type = CMM_ROUTE6;
		memcpy(&data.r_nexthop6, gwAddr, sizeof(struct in6_addr));
		memcpy(&data.r_addr6, dAddr, sizeof(struct in6_addr));
	}
	fpr_send_message(ctx->unix_sock, &data);
#endif
	...
	for (i = 0; i < ROUTE_HASH_TABLE_SIZE * 2; i++)
	{
		for (entry = list_first(&rt_table[i]); next_entry = list_next(entry), entry != &rt_table[i]; entry = next_entry)
		{
			...
			if (cmmPrefixEqual(route->dAddr, dAddr, rtm->rtm_dst_len))
			{
				...
				__cmmRouteUpdate(ctx->cpal_handle, route);
			}
		}
	}
	...
	__cmmRouteNew(ctx->cpal_handle, rtm, dAddr, flushed);
