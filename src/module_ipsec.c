...
int __cmmSATunnelRegister(cpal_handle_t *cpal_handle, struct SATable* SAEntry)
{
...
*/
	...
	cmmFeRouteUpdate(cpal_handle, ADD | UPDATE, SAEntry->tnl_rt.fpp_route);
...
program:
	...
	{
		if (cmmKeyEnginetoIPSec(cpal_handle, FPP_CMD_IPSEC_SA_TNL_ROUTE, sizeof(CommandIPSecSetTunnelRoute),(unsigned short*) &cmd_set_tnl_route) < 0)
		{
...
static void __cmmSARouteUpdate(cpal_handle_t *cpal_handle, struct SATable *s, struct RtEntry *route)
{
	...
	__cmmRouteDeregister(cpal_handle, &rt, "sa");
}

void __cmmSAUpdateWithRoute(cpal_handle_t *cpal_handle, struct RtEntry *route)
{
	struct SATable *s;
	struct list_head *entry;
	int i;
	...
	for (i = 0; i < SA_HASH_TABLE_SIZE; i++)
	{
		for (entry = list_first(&sa_table[i]); entry != &sa_table[i]; entry = list_next(entry))
		{
			...
			if (s->tnl_rt.route == route)
				 __cmmSARouteUpdate(cpal_handle, s, route);
		}
	}
...
int cmmSACreate(cpal_handle_t *cpal_handle, unsigned short fcode, unsigned short len, unsigned short *payload)
{
...
int cmmSADelete(cpal_handle_t *cpal_handle, PCommandIPSecDeleteSA pSA_cmd)
{
	struct SATable *pSAEntry;
	int rc = 0;
	__pthread_mutex_lock(&sa_lock);
	...
	__cmmSARemove(cpal_handle, pSAEntry);
...
int cmmSAFlush(cpal_handle_t *cpal_handle, unsigned short fcode, unsigned short len, unsigned short *payload)
{
	int i, rc = 0;
	struct SATable *pSAEntry;
	struct list_head *entry;
...
int cmmSASetOffloadState(cpal_handle_t *cpal_handle, unsigned short sagd, bool offload_status)
{
	...
	if (cmmKeyEnginetoIPSec(cpal_handle, FPP_CMD_IPSEC_SA_ACTION_OFFLOAD, sizeof(struct nlkey_sa_notify),
...
int cmmSASetState(cpal_handle_t *cpal_handle, unsigned short fcode, unsigned short len, unsigned short *payload,
						int *state_valid, unsigned short *sgid)
{
	...
	rc = cmmSASetOffloadState(cpal_handle, pSA_cmd->sagd, IS_TUNNEL_MODE_SA(pSAEntry));
#else
	rc = cmmSASetOffloadState(cpal_handle, pSA_cmd->sagd, true);
#endif
	if(rc < 0)
		goto out;
...
int cmmSASetTunnel(cpal_handle_t *cpal_handle, unsigned short fcode, unsigned short len, unsigned short *payload)
{
	...
	rc = __cmmSATunnelRegister(cpal_handle, pSAEntry);
out:	
#ifdef IPSEC_FLOW_CACHE
	__pthread_mutex_unlock(&flowMutex);
#endif /* IPSEC_FLOW_CACHE */
