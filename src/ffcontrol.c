...
*
******************************************************************/
static int cmmFcIsLoopback(cpal_handle_t *cpal_handle, struct nf_conntrack *ct, struct flow *flow_orig, struct RtEntry **rtEntryOrig)
{
...
int cmmFcIsConntrack4Allowed(cpal_handle_t *cpal_handle, struct nf_conntrack *ct, struct RtEntry **rtEntryOrig)
{
	...
	if (cmmFcIsLoopback(cpal_handle, ct, &flow_orig, rtEntryOrig)) {
...
*
******************************************************************/
int cmmFcIsConntrack6Allowed(cpal_handle_t *cpal_handle, struct nf_conntrack * ct, struct RtEntry **rtEntryOrig)
{
	struct denyRuleList * temp;
	denyRule_t	tempRule;
	struct flow flow_orig;
...
int cmmFcActivate(struct cli_def * cli, const char *command, char *argv[], int argc)
{
	...
	if (globalConf.enable != val)
	{
		if (val == 0)
		{
			...
			cmmFeReset(globalConf.cli.cpal_handle);
...
int cmmCliInit(struct cmm_cli *ctx)
{
	...
	if (!ctx->cpal_handle)
	{
		cmm_print(DEBUG_ERROR, "%s: cpal_ff_open() failed, %s\n", __func__, strerror(errno));
		goto err0;
	}
#endif
#ifdef NEW_IPC
