...
int cmmMc6ProcessClientCmd(cpal_handle_t* cpal_handle, int function_code, u_int8_t *cmd_buf, u_int16_t cmd_len, u_int16_t *res_buf, u_int16_t *res_len)
{
	int ret = 0;
...
#ifndef DPDK_ENABLE
			res_buf[0] = cpal_write(cpal_handle, CMMD_CMD_MC6_RESET, 0, NULL);
			if ( ret == CMMD_ERR_OK )
				mc_reset(AF_INET6);
#endif
			__pthread_mutex_unlock(&mc_lock);
