...
int cmmMc4ProcessClientCmd(cpal_handle_t* cpal_handle, int function_code, u_int8_t *cmd_buf, u_int16_t cmd_len, u_int16_t *res_buf, u_int16_t *res_len)
{
	int ret = 0;
...
out:
		...
		case CMMD_CMD_MC4_RESET:
			...
			res_buf[0] = cpal_write(cpal_handle, CMMD_CMD_MC4_RESET, 0, NULL);
			if ( ret == CMMD_ERR_OK )
				mc_reset(AF_INET);		
...
CPAL_CMD:
	return cpal_cmd(cpal_handle, function_code, (u_int16_t*)cmd_buf, cmd_len, res_buf, res_len);
}
