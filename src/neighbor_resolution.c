...
int cmm_br_get_neigh(char *lladdr, uint32_t br_ifindex, uint16_t vlan)
{
	...
	cmm_print(DEBUG_STDOUT, "%s: outgoing interaface is %s ret= %d\n", __func__, if_indextoname(out_index,ifname), rc);
...
*
******************************************************************/
static void __cmmNeighborResolved(cpal_handle_t *cpal_handle, struct NeighborEntry *neigh)
{
	...
	while (entry != &rt_table_by_gw_ip[key])
	{
		...
		__cmmCtUpdateWithRoute(cpal_handle, route);

		__cmmTunnelUpdateWithRoute(cpal_handle, route);

		__cmmSocketUpdateWithRoute(cpal_handle, route);
...
*
******************************************************************/
static void __cmmNeighborUnresolved(cpal_handle_t *cpal_handle, struct NeighborEntry *neigh)
{
	/* Process conntrack entries that use this neighbor */
	struct RtEntry *route;
	struct list_head *entry;
	...
	cmm_print(DEBUG_INFO, "%s: Remove ARP/Neighbor entry for mac:%s\n", __func__, neigh->list_by_mac);
...
static int cmmNeighborUpdate(struct cmm_ct *ctx, const struct sockaddr_nl *who, struct nlmsghdr *n)
{
	...
	if (n->nlmsg_type == RTM_NEWNEIGH) {
		data.n_action = NEIGHBOR_ADD;
	} else {
		data.n_action = NEIGHBOR_DELETE;
	}
	data.n_portid = r->ndm_ifindex;
	if (r->ndm_family == AF_INET) {
		data.msg_type = CMM_NEIGHBOR4;
		memcpy(&data.n_addr, ipAddr, sizeof(struct in_addr));
	} else if (r->ndm_family == AF_INET6) {
		data.msg_type = CMM_NEIGHBOR6;
		memcpy(&data.n_addr6, ipAddr, sizeof(struct in6_addr));
	}
	if (macAddr)
		memcpy(data.n_mac, macAddr, sizeof(uint8_t) * MAX_MAC_LENGTH);
	else
		cmm_print(DEBUG_CRIT, "%s: No mac addr available\n", __func__);
	data.n_flags = r->ndm_state;
//	data.vlanid =  *(__u16 *) RTA_DATA(tb[NDA_VLAN]);
	data.n_vlanid = 0;
	...
	if (r->ndm_state & NUD_VALID)
	{
		...
		{
			if (macAddrLen > sizeof(neigh->macAddr))
				macAddrLen = sizeof(neigh->macAddr);
			memcpy(neigh->macAddr, macAddr, macAddrLen);

			__cmmNeighborResolved(ctx->cpal_handle, neigh);
		}
		else
		{
			if (macAddrLen > sizeof(neigh->macAddr))
	...
	else
	{
		...
		if (old_state & NUD_VALID)
			__cmmNeighborUnresolved(ctx->cpal_handle, neigh);
	}
