...
*
******************************************************************/
void cmmFeReset(cpal_handle_t *cpal_handle)
{
	int i;
...
#ifndef DPDK_ENABLE
	if (cpal_write(cpal_handle, FPP_CMD_IPV4_RESET, 0, NULL))
	{
...
#ifndef DPDK_ENABLE
	if (cpal_write(cpal_handle, FPP_CMD_IPV6_RESET, 0, NULL))
	{
...
#ifdef AUTO_BRIDGE
	 __cmm_l2flow_reset(cpal_handle);
#endif
...
static void cmmFeL2FlowChange(struct cmm_ct *ctx, fpp_l2_bridge_flow_entry_cmd_t* cmd, int len)
{
	...
	__pthread_mutex_lock(&brMutex);
	__cmm_l2flow_deregister(ctx->cpal_handle, &l2flow_tmp);
	__pthread_mutex_unlock(&brMutex);

}

int cmmFeL2FlowUpdate(cpal_handle_t* cpal_handler, int request, struct l2flowTable *l2flow_entry)
{
	int action;
	short ret;
...
#ifndef DPDK_ENABLE
		ret = cpal_write(cpal_handler, FPP_CMD_RX_L2FLOW_ENTRY, sizeof(fpp_l2_bridge_flow_entry_cmd_t), (unsigned short *) &cmd);
...
*
******************************************************************/
int cmmFeCtUpdate4(cpal_handle_t *cpal_handler, int action, struct ctTable *ctEntry)
{
	fpp_ct_ex_cmd_t cmd;
...
#ifndef DPDK_ENABLE
		ret = cpal_write(cpal_handler, FPP_CMD_IPV4_CONNTRACK, cmd_size, (unsigned short *) &cmd);
...
#ifndef DPDK_ENABLE
		ret = cpal_write(cpal_handler, FPP_CMD_IPV4_CONNTRACK, sizeof(fpp_ct_cmd_t), (unsigned short *) &cmd);
...
*
******************************************************************/
int cmmFeCtUpdate6(cpal_handle_t *cpal_handler, int action, struct ctTable *ctEntry)
{
...
#ifndef DPDK_ENABLE
		ret = cpal_write(cpal_handler, FPP_CMD_IPV6_CONNTRACK, cmd_size, (unsigned short *) &cmd);
...
#ifndef DPDK_ENABLE
		ret = cpal_write(cpal_handler, FPP_CMD_IPV6_CONNTRACK, sizeof(fpp_ct6_cmd_t), (unsigned short *) &cmd);
		if ((ret == FPP_ERR_OK) || (ret == FPP_ERR_CT_ENTRY_NOT_FOUND))
		{
#ifdef  IPSEC_FLOW_CACHE 
			if (ctEntry->fEntryOrigOut)
...
*
******************************************************************/
int __cmmFeRouteUpdate(cpal_handle_t* cpal_handler, int action, struct fpp_rt *fpp_route)
{
	fpp_rt_cmd_t cmd;
	short ret;
...
#ifndef DPDK_ENABLE
		ret = cpal_write(cpal_handler, FPP_CMD_IP_ROUTE, sizeof(fpp_rt_cmd_t), (unsigned short *)&cmd);
...
#endif
int cmmFeFFControl(cpal_handle_t* cpal_handler, u_int8_t *cmd_buf, u_int16_t cmd_len, u_int16_t *res_buf, u_int16_t *res_len)
{
	...
	*res_len = 2;
	...
	if(cmd->enable)
	{
		if(!globalConf.ff_enable)
		{
			...
			errno = 0;
			ret = cpal_cmd(cpal_handler, FPP_CMD_IPV4_FF_CONTROL, (unsigned short *) cmd, sizeof(fpp_ff_ctrl_cmd_t), res_buf, res_len);
...
int cmmFeCatch(unsigned short fcode, unsigned short len, unsigned short *payload)
{
	...
	case FPP_CMD_IPSEC_SA_NOTIFY:
		...
		cmmIPSectoKeyEngine(globalConf.ct.cpal_key_handle, fcode, len, payload);
		break;
#ifdef AUTO_BRIDGE
