...
int voice_file_load(cpal_handle_t *cpal_handle, cmmd_voice_file_load_cmd_t *cmd, u_int16_t *res_buf, u_int16_t *res_len)
{
	...
	*res_len = 2;
	...
	rc = cpal_cmd(cpal_handle, FPP_CMD_VOICE_BUFFER_LOAD, (u_int16_t *) &fpp_cmd, sizeof(fpp_cmd), res_buf, res_len);
...
int voice_file_unload(cpal_handle_t *cpal_handle, cmmd_voice_file_unload_cmd_t *cmd, u_int16_t *res_buf, u_int16_t *res_len)
{
	...
	*res_len = 2;
	...
	rc = cpal_cmd(cpal_handle, FPP_CMD_VOICE_BUFFER_UNLOAD, (u_int16_t *) &fpp_cmd, sizeof(fpp_cmd), res_buf, res_len);
	if (rc == 0 && res_buf[0] == FPP_ERR_OK)
	{
...
#ifndef DPDK_ENABLE
	rc = cpal_write(cpal_handle, FPP_CMD_VOICE_BUFFER_RESET, 0, NULL);
#endif
