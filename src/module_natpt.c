...
int cmmNATPTOpenProcessClientCmd(cpal_handle_t* cpal_handle, u_int8_t *cmd_buf, u_int16_t cmd_len, u_int16_t *res_buf, u_int16_t *res_len)
{
	...
	if (socka_rtentry && sockb_rtentry)
	{
		...
		__socket_open(cpal_handle, sockA);
		...
		__socket_open(cpal_handle, sockB);
		__cmmRouteDeregister(cpal_handle, &old_route, "socket");
	}

	rc = cpal_cmd(cpal_handle, FPP_CMD_NATPT_OPEN, (unsigned short *)cmd_buf, cmd_len, (unsigned short *)res_buf, res_len);
out:
