...
void
fpr_send_message(int fd, struct cmm_msg *message)
{
    pthread_mutex_lock(&send_mutex);
    send(fd, message, sizeof(struct cmm_msg), 0);
    pthread_mutex_unlock(&send_mutex);
    cmm_print(DEBUG_INFO, "%s: message sent to FPR\n", __func__);
}
...
err1:
	...
	if (ret < 0)
		cmm_print(DEBUG_STDERR, "%s: file %s removal failed: %s\n", __func__, CMM_PID_FILE_PATH, strerror(errno));
err0:
	cmm_print(DEBUG_CRIT, "%s: exiting\n", __func__);
