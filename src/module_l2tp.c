...
#ifndef DPDK_ENABLE
	rc = cpal_write(cpal_handle, FPP_CMD_L2TP_ITF_ADD, sizeof(cmd), (unsigned short *) &cmd);
	if (rc == FPP_ERR_OK)
	{
...
#ifndef DPDK_ENABLE
	rc = cpal_write(cpal_handle, FPP_CMD_L2TP_ITF_DEL, sizeof(cmd), (unsigned short *) &cmd);
	if (rc == FPP_ERR_OK)
	{
		itf->flags &= ~FPP_PROGRAMMED;
		itf->flags &= ~FPP_NEEDS_UPDATE;
...
int l2tp_itf_del(cpal_handle_t *cpal_handle, struct interface *itf)
{
	...
	rc = __l2tp_itf_del(cpal_handle, itf);

	if(itf->l2tp.sock)
	{
		__socket_close(cpal_handle,itf_table.cpal_key_handle,s);
		itf->l2tp.sock =NULL;
...
int l2tp_new_session(cpal_handle_t *cpal_handle, cmmd_l2tp_session_t *cmd, u_int16_t *res_buf, u_int16_t *res_len)
{
	int ifindex;
	...
	{
		/* Close stale socket */		
		rc = __socket_close(cpal_handle, itf_table.cpal_key_handle, itf->l2tp.sock);
	...
	rc = l2tp_itf_add(cpal_handle, ADD, itf);
	if(rc != CMMD_ERR_OK) {
		itf->flags &= ~USER_ADDED;
		goto err;
	}
...
int l2tp_delete_session(cpal_handle_t *cpal_handle, cmmd_l2tp_session_t *cmd, u_int16_t *res_buf, u_int16_t *res_len)
{
	int rc = 0;
	struct interface *itf;
	int ifindex;
	...
	if (__itf_is_l2tp(itf))
		rc = l2tp_itf_del(itf_table.cpal_handle, itf);
err:
	__pthread_mutex_unlock(&socket_lock);
#ifdef IPSEC_FLOW_CACHE
	__pthread_mutex_unlock(&flowMutex);
...
int l2tp_daemon(cpal_handle_t *cpal_handle,int command, cmmd_l2tp_session_t *cmd,  u_int16_t cmd_len, u_int16_t *res_buf, u_int16_t *res_len)
{
	...
	*res_len = 2;
	...
	if(command == CMMD_CMD_L2TP_SESSION_CREATE)
		return l2tp_new_session(cpal_handle, cmd, res_buf, res_len);
	else if(command == CMMD_CMD_L2TP_SESSION_DESTROY)
		return l2tp_delete_session(cpal_handle, cmd, res_buf, res_len);
	else
	{
