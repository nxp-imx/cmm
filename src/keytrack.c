...
*
******************************************************************/
int cmmKeyEnginetoIPSec(cpal_handle_t *cpal_handle, unsigned short fcode, unsigned short len, unsigned short *payload)
{
	int rc = 0;

	cmm_print(DEBUG_INFO, "%s: fcode 0x%x len %d bytes\n", __func__, fcode, len);
#ifndef DPDK_ENABLE
	if ((rc = cpal_write(cpal_handle, fcode, len, payload)))
	{
...
*
******************************************************************/
int cmmIPSectoKeyEngine(cpal_handle_t *cpal_handle, unsigned short fcode, unsigned short len, unsigned short *payload)
{
	int rc = 0;
...
*
******************************************************************/
int cmmFlowKeyEngineRemove(cpal_handle_t *cpal_handle, struct FlowEntry *fentry)
{
	...
	return cmmIPSectoKeyEngine(cpal_handle, FPP_CMD_NETKEY_FLOW_REMOVE, len, (unsigned short *)msg);
}
				...
				unsigned short flsport, unsigned short fldport, int dir )
{
	...
	if (ctEntry)
	{
		...
		____cmmCtRegister(globalConf.ct.cpal_handle, ctEntry);
	}
...
*
******************************************************************/
int cmmKeyEngineFlow2Conntrack(cpal_handle_t *cpal_handle, unsigned short fcode, unsigned short len, unsigned short *payload)
{
...
*
******************************************************************/
int cmmKeyEngineFlowRemove(cpal_handle_t *cpal_handle, unsigned short fcode, unsigned short len, unsigned short *payload)
{
	unsigned short family;
	unsigned short dir;
	unsigned short msg_len = 0;
...
int cmmKeyCatch(unsigned short fcode, unsigned short len, unsigned short *payload)
{
	...
	switch (fcode)
	{
		case FPP_CMD_NETKEY_SA_ADD:
			if(cmmSACreate(globalConf.ct.cpal_handle, fcode, len, payload) < 0)
				rc = CPAL_CB_STOP;
			else if(cmmKeyEnginetoIPSec(globalConf.ct.cpal_handle, fcode, len, payload) < 0)
				rc = CPAL_CB_STOP;
			break;
		case FPP_CMD_NETKEY_SA_DELETE:
			{
				...
				if(cmmSADelete(globalConf.ct.cpal_handle, pSA_cmd) < 0)
					rc = CPAL_CB_STOP;
				if(cmmKeyEnginetoIPSec(globalConf.ct.cpal_handle, fcode, len, payload) < 0)
					rc = CPAL_CB_STOP;
			}
			break;
		case FPP_CMD_NETKEY_SA_FLUSH:
			if(cmmKeyEnginetoIPSec(globalConf.ct.cpal_handle, fcode, len, payload) < 0)
				rc = CPAL_CB_STOP;
			if(cmmSAFlush(globalConf.ct.cpal_handle, fcode, len, payload) < 0)
				rc = CPAL_CB_STOP;
			break;
		case FPP_CMD_NETKEY_SA_SET_TUNNEL:
			if(cmmKeyEnginetoIPSec(globalConf.ct.cpal_handle, fcode, len, payload) < 0)
				rc = CPAL_CB_STOP;
			else if(cmmSASetTunnel(globalConf.ct.cpal_handle, fcode, len, payload) < 0)
				rc = CPAL_CB_STOP;
			break;
		...
		case FPP_CMD_NETKEY_SA_SET_STATE:
			if(cmmSASetState(globalConf.ct.cpal_key_handle, fcode, len, payload, &state_valid, &sgid)< 0)
				rc = CPAL_CB_STOP;
			else if (cmmKeyEnginetoIPSec(globalConf.ct.cpal_handle, fcode, len, payload) < 0)
			{
				rc = CPAL_CB_STOP;
		...
		case FPP_CMD_NETKEY_FLOW_ADD:
			cmmKeyEngineFlow2Conntrack(globalConf.ct.cpal_handle, fcode, len, payload);
			break;

		case FPP_CMD_NETKEY_FLOW_REMOVE:
			cmmKeyEngineFlowRemove(globalConf.ct.cpal_handle, fcode, len, payload);
			break;
#endif /* IPSEC_FLOW_CACHE */
...
void cmmDPDIPsecSAUpdate(struct cmm_ct *ctx)
{
...
#endif
	{
		...
		ret = cpal_write(ctx->cpal_handle, FPP_CMD_STAT_IPSEC_STATUS, sizeof(ipsecStatusCmd), (unsigned short *) &ipsecStatusCmd);
		...
		else
		{
			while (1)
			{
				ret = cpal_query(ctx->cpal_handle,FPP_CMD_STAT_IPSEC_ENTRY,0,NULL,
...
#endif
				{
					...
					payload = (unsigned short *)&msg;
			                cmmIPSectoKeyEngine(ctx->cpal_key_handle, fcode, len, payload);
				}
#if defined (LS1043)
