...
static void cmmCtSetPermanent(struct nfct_handle * handler, int flags,  struct nf_conntrack * ctTemp, struct nf_conntrack *ct, int reset)
{
	...
	if (flags & LOCAL_CONN)
	{
		cmm_print(DEBUG_INFO, "%s: local connection\n",__func__);
		return;
	}

	if (!ctTemp)
	{
		cmm_print(DEBUG_INFO, "%s: Conntrack not Cloned\n",__func__);
		return;
	} 
	...
	if (((status & IPS_PERMANENT) == 0) && (flags & (FPP_PROGRAMMED)))
	{
		...
		cmm_print(DEBUG_ERROR, "%s: invalid use case (status:%d flags:%d)\n",__func__, status, flags);
		return;
	}
	...
	for (i = 0 ; i < CONNTRACK_HASH_TABLE_SIZE; i++)
	{
		...
		for(entry = list_first(&ct_table[i]); entry != &ct_table[i]; entry = list_next(entry))
		{
			...
			if (temp->flags & FPP_PROGRAMMED)
				cmmFeGetTimeout(globalConf.cli.cpal_handle, temp, &timeout);
			else
				timeout = 0;
...
*
******************************************************************/
int ____cmmCtLocalRegister(cpal_handle_t *cpal_handle, struct ctTable* ctEntry)
{
	lro_socket_open(cpal_handle, ctEntry);

	/* Update all dynamic connections/ tunnel routes for which tunnel route
	is not attached */
	__cmmRouteLocalNew(cpal_handle, ctEntry);
	return 0;
}
...
*
******************************************************************/
int ____cmmCtRegister(cpal_handle_t *cpal_handle, struct ctTable *ctEntry)
{
	struct nf_conntrack *ct = ctEntry->ct;
...
//		printf(" in route originator = %p\n", ctEntry->orig.route);
		...
		{
			dir &= ~ORIGINATOR;
			cmm_print(DEBUG_INFO,"%s:__cmmRouteRegister(originator) failed\n", __func__);
			goto replier;
		}
		...
		if (ctEntry->dir_filter & ORIGINATOR) {
			...
			if (!(__itf_is_wifi(out_itf) && (!is_wan_port_ifindex(iif))) &&
				...
				dir &= ~ORIGINATOR;
				cmm_print(DEBUG_INFO,"%s: forward from/to WLAN \n", __func__);
				goto replier;
			}
		}
...
program:
	if (dir & ORIGINATOR)
	{
		rc = cmmFeRouteUpdate(cpal_handle, ADD | UPDATE, ctEntry->orig.fpp_route);
		if (rc < 0)
		{
			dir &= ~ORIGINATOR;
			cmm_print(DEBUG_ERROR,"%s: cmmFeRouteUpdate(ADD|UPDATE|originator) failed\n", __func__);
			goto program_replier;
		}
		cmm_print(DEBUG_ERROR,"%s: cmmFeRouteUpdate(ADD|UPDATE|originator) success\n", __func__);

		if (ctEntry->orig_tunnel.fpp_route)
		{
			rc = cmmFeRouteUpdate(cpal_handle, ADD | UPDATE, ctEntry->orig_tunnel.fpp_route);
			if (rc < 0)
			{
				dir &= ~ORIGINATOR;
				cmm_print(DEBUG_ERROR,"%s: cmmFeRouteUpdate(ADD|UPDATE|originator/tunel) failed\n", __func__);
				goto program_replier;
			}
			cmm_print(DEBUG_ERROR,"%s: cmmFeRouteUpdate(ADD|UPDATE|originator/tunel) success\n", __func__);
		}
	}
...
program_replier:
	...
	if (dir & REPLIER)
	{
		rc = cmmFeRouteUpdate(cpal_handle, ADD | UPDATE, ctEntry->rep.fpp_route);
		if (rc < 0)
		{
			dir &= ~REPLIER;
			cmm_print(DEBUG_ERROR,"%s: cmmFeRouteUpdate(ADD|UPDATE|replier) failed\n", __func__);
			goto program_ct;
		}
		cmm_print(DEBUG_ERROR,"%s: cmmFeRouteUpdate(ADD|UPDATE|replier) success\n", __func__);

		if (ctEntry->rep_tunnel.fpp_route)
		{
			rc = cmmFeRouteUpdate(cpal_handle, ADD | UPDATE, ctEntry->rep_tunnel.fpp_route);
			if (rc < 0)
			{
				dir &= ~REPLIER;
				cmm_print(DEBUG_ERROR,"%s: cmmFeRouteUpdate(ADD|UPDATE|replier/tunel) failed\n", __func__);
				goto program_ct;
			}
			cmm_print(DEBUG_ERROR,"%s: cmmFeRouteUpdate(ADD|UPDATE|replier/tunel) success\n", __func__);
		}
	}
...
program_ct:
	...
	if (dir) {
		cmm_print(DEBUG_INFO, "%s: cmmFeCtUpdate(ADD|UPDATE)\n", __func__);
		rc = cmmFeCtUpdate(cpal_handle, ADD | UPDATE, ctEntry);
	}
	else {
		cmm_print(DEBUG_INFO, "%s: cmmFeCtUpdate(UPDATE)\n", __func__);
		rc = cmmFeCtUpdate(cpal_handle, UPDATE, ctEntry);
	}
...
*
******************************************************************/
void __cmmCtUpdateWithRoute(cpal_handle_t *cpal_handle, struct RtEntry *route)
{
	struct ctTable *ctEntry;
	struct list_head *entry;
	...
	while (entry != &ct_table_by_orig_route[key])
	{
		...
		if (ctEntry->orig.route == route)
		{
			...
			__cmmFPPRouteDeregister(cpal_handle, fpp_route, "originator");
		}
	}
	...
	while (entry != &ct_table_by_rep_route[key])
	{
		...
		if (ctEntry->rep.route == route)
		{
			...
			__cmmFPPRouteDeregister(cpal_handle, fpp_route, "replier");
		}
	}
	...
	while (entry != &ct_table_by_orig_tunnel_route[key])
	{
		...
		if (ctEntry->orig_tunnel.route == route)
		{
			...
			__cmmFPPRouteDeregister(cpal_handle, fpp_route, "originator tunnel");
		}
        }
	...
	while (entry != &ct_table_by_rep_tunnel_route[key])
	{
		...
		if (ctEntry->rep_tunnel.route == route)
		{
			...
			__cmmFPPRouteDeregister(cpal_handle, fpp_route, "replier tunnel");
		}
	}
}
...
*
******************************************************************/
static int __cmmCtRegister(cpal_handle_t *cpal_handle, struct nfct_handle *handle, struct nf_conntrack *ct, struct ctTable *ctEntry, int dir)
{
	...
	if (!ctEntry)
	{
		if (!cmmFcIsConntrackAllowed(cpal_handle, ct, &rtEntryOrig))
		{
...
*
******************************************************************/
void __cmmFPPRouteDeregister(cpal_handle_t *cpal_handle, struct fpp_rt *fpp_route, const char *dir)
{
	int rc = 0;

	if (!fpp_route) {
		cmm_print(DEBUG_ERROR, "%s: Error no fpp_route\n", __func__);
		return;
	}
	...
	{
		cmm_print(DEBUG_INFO, "%s: cmmFeRouteUpdate(REMOVE)\n", __func__);
		rc = cmmFeRouteUpdate(cpal_handle, REMOVE, fpp_route);
	}
...
*
******************************************************************/
void __cmmRouteDeregister(cpal_handle_t *cpal_handle, struct ct_route *rt, const char *dir)
{
	if (rt->fpp_route)
	{
		__cmmFPPRouteDeregister(cpal_handle, rt->fpp_route, dir);
		rt->fpp_route = NULL;
	}
...
*
******************************************************************/
int  ____cmmCtLocalDeregister(cpal_handle_t *cpal_handle, cpal_handle_t *cpal_key_handle, struct ctTable *ctEntry)
{
	/* As the local route is attached to ctEntry for local connections */
	__pthread_mutex_lock(&rtMutex);
	__pthread_mutex_lock(&neighMutex);

	__cmmRouteDeregister(cpal_handle, &ctEntry->orig, "originator");
	__cmmRouteDeregister(cpal_handle, &ctEntry->rep, "replier");

	lro_socket_close(cpal_handle, cpal_key_handle, ctEntry);
...
*
******************************************************************/
int ____cmmCtDeregister(cpal_handle_t *cpal_handle, cpal_handle_t *cpal_key_handle, struct ctTable *ctEntry)
{
	int rc;

	if (ctEntry->flags & LOCAL_CONN)
	{
		rc = ____cmmCtLocalDeregister(cpal_handle,cpal_key_handle,ctEntry);
		goto ct_remove;
	}

	rc = cmmFeCtUpdate(cpal_handle, REMOVE, ctEntry);
	...
	{
		__cmmRouteDeregister(cpal_handle, &ctEntry->rep_tunnel, "replier tunnel");
		list_del(&ctEntry->list_by_rep_tunnel_route);
	}

	if (ctEntry->orig_tunnel.route)
	{
		__cmmRouteDeregister(cpal_handle, &ctEntry->orig_tunnel, "originator tunnel");
	...
	if (ctEntry->fEntryOrigOut)
		if (!cmmFlowKeyEngineRemove(cpal_key_handle, ctEntry->fEntryOrigOut))
		{
	...
	if (ctEntry->fEntryOrigFwd)
		if (!cmmFlowKeyEngineRemove(cpal_key_handle, ctEntry->fEntryOrigFwd))
		{
	...
	if (ctEntry->fEntryRepOut)
		if (!cmmFlowKeyEngineRemove(cpal_key_handle, ctEntry->fEntryRepOut))
		{
	...
	if (ctEntry->fEntryRepFwd)
		if (!cmmFlowKeyEngineRemove(cpal_key_handle, ctEntry->fEntryRepFwd))
		{
...
static void __cmmCtDeregister(struct cmm_ct *ctx, struct nf_conntrack *ct)
{
	...
	ct_stats.destroyed++;
	____cmmCtDeregister(ctx->cpal_handle, ctx->cpal_key_handle, ctEntry);
		...
		case NFCT_T_UPDATE:
			...
			if (l4proto == IPPROTO_UDP) {
				if (status & IPS_ASSURED) {
					...
					rc = __cmmCtRegister(ctx->cpal_handle, ctx->handle, ct, ctEntry, ORIGINATOR);
				}
				else {
					goto exit;
				}
			...
			if (nfct_attr_is_set(ct, ATTR_TCP_STATE))
			{
				...
				if ((state == TCP_CONNTRACK_ESTABLISHED) && (status & IPS_ASSURED))
					...
					if (ctEntry)
					{
						...
						if(!ctTemp)
							ctTemp = cmmCtClone(ct);
						if(!____cmmCtDeregister(ctx->cpal_handle, ctx->cpal_key_handle, ctEntry))
...
static void *cmmCtThread(void *data)
{
...
#ifndef DPDK_ENABLE
#if !defined(IPSEC_SUPPORT_DISABLED)
	fd_key = cpal_fd(ctx->cpal_key_catch_handle);
#endif
	fd_cpal = cpal_fd(ctx->cpal_catch_handle);
#endif
	...
	while (1)
	{
		...
		{
			rc = cpal_catch(ctx->cpal_key_catch_handle);
			if (rc < 0)
			{
				if (errno != EAGAIN)
				{
					cmm_print(DEBUG_ERROR, "%s: cpal_catch() failed %s\n", __func__, strerror(errno));
				}
			}
		}
#endif
...
#ifndef DPDK_ENABLE
		if (FD_ISSET(fd_cpal, &set))
		{
			rc = cpal_catch(ctx->cpal_catch_handle);
			if (rc < 0)
			{
				if (errno != EAGAIN)
				{
...
int cmmCtInit(struct cmm_ct *ctx)
{
	...
	{
		cmm_print(DEBUG_CRIT, "%s: clone fails\n", __func__);
		goto err0;
	}
	...
	{
		cmm_print(DEBUG_CRIT, "%s: update bufs fails\n", __func__);
		goto err0;
	}
	...
	{
		cmm_print(DEBUG_CRIT, "%s: kmodule init fails\n", __func__);
		goto err0;
	}
#endif
#ifdef APP_SOLICIT
	...
	{
		cmm_print(DEBUG_CRIT, "%s: rtnl kmodule fails fails\n", __func__);
		goto err1;
	}
#endif
#ifndef DPDK_ENABLE
	ctx->cpal_catch_handle = cpal_ff_catch_open();
	if (!ctx->cpal_catch_handle)
	{
		cmm_print(DEBUG_CRIT, "%s:%d cpal_ff_catch_open() failed, %s\n", __func__, __LINE__, strerror(errno));
		goto err2;
	}

	ctx->cpal_handle = cpal_ff_open();
	if (!ctx->cpal_handle)
	{
		cmm_print(DEBUG_CRIT, "%s:%d cpal_ff_open() failed, %s\n", __func__, __LINE__, strerror(errno));
		goto err3;
	}

#if !defined(IPSEC_SUPPORT_DISABLED)
	ctx->cpal_key_catch_handle = cpal_key_catch_open();
	if (!ctx->cpal_key_catch_handle)
	{
		cmm_print(DEBUG_CRIT, "%s:%d cpal_key_catch_open() failed, %s\n", __func__, __LINE__, strerror(errno));
		goto err4;
	}

	ctx->cpal_key_handle = cpal_key_open();
	if (!ctx->cpal_key_handle)
	{
		cmm_print(DEBUG_CRIT, "%s:%d cpal_key_open() failed, %s\n", __func__, __LINE__, strerror(errno));
		goto err5;
	}
#endif
#else
	...
	if(cmmBridgeInit(ctx)) {
		cmm_print(DEBUG_CRIT, "%s: bridge int fails\n", __func__);
		goto err14;
	}
#ifndef DPDK_ENABLE
	fd = cpal_fd(ctx->cpal_catch_handle);
...
#if !defined(IPSEC_SUPPORT_DISABLED)
	fd = cpal_fd(ctx->cpal_key_catch_handle);
...
#ifndef DPDK_ENABLE
	cpal_register_cb(ctx->cpal_catch_handle, cmmFeCatch);
#if !defined(IPSEC_SUPPORT_DISABLED)
	cpal_register_cb(ctx->cpal_key_catch_handle, cmmKeyCatch);
#endif
#endif
	...
	for (i = 0; i < GEM_PORTS; i++) {
		if (itf_name_update(ctx->cpal_handle, &port_table[i]) < 0)
			goto err14;
	}
#endif
#endif
	...
	cmm_print(DEBUG_STDOUT, "%s: CMM is loaded\n", __func__);
...
*
******************************************************************/
static int cmmCtChange(cpal_handle_t *cpal_handle, int function_code, u_int8_t *cmd_buf, u_int16_t *res_buf, u_int16_t *res_len)
{
	...
	rc = ____cmmCtRegister(cpal_handle, ctEntry);
	if (rc == 0)
	{
...
int cmmCtHandle(cpal_handle_t *cpal_handle, int function_code, u_int8_t *cmd_buf, u_int16_t cmd_len, u_int16_t *res_buf, u_int16_t *res_len)
{
	...
	case CMMD_ACTION_UPDATE:
		rc = cmmCtChange(cpal_handle, function_code, cmd_buf, res_buf, res_len);
		break;

	case CMMD_ACTION_QUERY:
	case CMMD_ACTION_QUERY_CONT:
		rc = cpal_cmd(cpal_handle, function_code, (u_int16_t*)cmd_buf, cmd_len, res_buf, res_len);
		break;
