 ...
 ************************************************************/
static int socket4_send_cmd(cpal_handle_t *cpal_handle, int action, struct socket *s)
{
	int ret = CMMD_ERR_OK;
...
#ifndef DPDK_ENABLE
		ret = cpal_write(cpal_handle, FPP_CMD_IPV4_SOCK_OPEN, sizeof(fpp_socket4_open_cmd_t), (u_int16_t *) &cmd);
		if (ret == FPP_ERR_OK)
		{
...
#ifndef DPDK_ENABLE
		ret = cpal_write(cpal_handle, FPP_CMD_IPV4_SOCK_UPDATE, sizeof(fpp_socket4_update_cmd_t), (u_int16_t *) &cmd);
		if (ret == FPP_ERR_OK)
		{
			s->flags &= ~FPP_NEEDS_UPDATE;
		}
...
#ifndef DPDK_ENABLE
		ret = cpal_write(cpal_handle, FPP_CMD_IPV4_SOCK_CLOSE, sizeof(fpp_socket4_close_cmd_t), (unsigned short *) &cmd);
		if (ret == FPP_ERR_OK)
		{
			s->flags &= ~FPP_PROGRAMMED;
		}
 ...
 * Role: CMM to FPP commands in deamon context
 ************************************************************/
static int socket6_send_cmd(cpal_handle_t *cpal_handle, int action, struct socket *s)
{
	int ret = CMMD_ERR_OK;
...
#ifndef DPDK_ENABLE
		ret = cpal_write(cpal_handle, FPP_CMD_IPV6_SOCK_OPEN, sizeof(fpp_socket6_open_cmd_t), (u_int16_t *) &cmd);
		if (ret == FPP_ERR_OK)
		{
			s->flags |= FPP_PROGRAMMED;
			s->flags &= ~FPP_NEEDS_UPDATE;
...
#ifndef DPDK_ENABLE
		ret = cpal_write(cpal_handle, FPP_CMD_IPV6_SOCK_UPDATE, sizeof(fpp_socket6_update_cmd_t), (u_int16_t *) &cmd);
		if (ret == FPP_ERR_OK)
		{
			s->flags &= ~FPP_NEEDS_UPDATE;
		}
...
#ifndef DPDK_ENABLE
		ret = cpal_write(cpal_handle, FPP_CMD_IPV6_SOCK_CLOSE, sizeof(fpp_socket6_close_cmd_t), (unsigned short *) &cmd);
		if (ret == FPP_ERR_OK)
		{
			s->flags &= ~FPP_PROGRAMMED;
		}
 ...
 * Role: CMM to FPP commands in deamon context
 ************************************************************/
static int socket_send_cmd(cpal_handle_t *cpal_handle, int request, struct socket *s)
{
	int action;
	...
	if (s->family == AF_INET)
		return socket4_send_cmd(cpal_handle, action, s);
	else if (s->family == AF_INET6)
		return socket6_send_cmd(cpal_handle, action, s);
	else
		return CMMD_ERR_UNKNOWN;
}

int __socket_open(cpal_handle_t *cpal_handle, struct socket *s)
{
	int rc;
	...
	if ((s->type == CMMD_SOCKET_TYPE_LANWAN) || (s->type == CMMD_SOCKET_TYPE_L2TP))
	{
		...
		cmmFeRouteUpdate(cpal_handle, ADD | UPDATE, s->rt.fpp_route);
	}
...
program:
	...
	rc = socket_send_cmd(cpal_handle, ADD | UPDATE, s);
 ...
 *
 ************************************************************/
void __cmmSocketUpdateWithRoute(cpal_handle_t *cpal_handle, struct RtEntry *route)
{
	struct socket *s;
	struct list_head *entry;
	...
	for (i = 0; i < HASH_SOCKET_SIZE; i++)
	{
		for (entry = list_first(&socket_table[i]); entry != &socket_table[i]; entry = list_next(entry))
		{
			...
			if (s->rt.route == route)
			{
				...
				__socket_open(cpal_handle, s);

				__cmmFPPRouteDeregister(cpal_handle, fpp_route, "socket");
			}
		}
	}
...
int __socket_close(cpal_handle_t *cpal_handle, cpal_handle_t *cpal_key_handle, struct socket *s)
{
	int rc = socket_send_cmd(cpal_handle, REMOVE, s);
	if (rc != CMMD_ERR_OK)
		return rc;

	__cmmRouteDeregister(cpal_handle, &s->rt, "socket");

#if defined(COMCERTO_2000) || defined(LS1043)
#ifdef IPSEC_FLOW_CACHE
	if (s->rx_flow)
		if (!cmmFlowKeyEngineRemove(cpal_key_handle, s->rx_flow))
		{
			__cmmFlowPut(s->rx_flow);
			s->rx_flow = NULL;
		}
	if (s->tx_flow)
		if (!cmmFlowKeyEngineRemove(cpal_key_handle, s->tx_flow))
		{
...
static int socket_open(cpal_handle_t *cpal_handle, cpal_handle_t *cpal_key_handle, cmmd_socket_open_cmd_t *cmd)
{
	struct socket *s;
	...
	rc = __socket_open(cpal_handle, s);
	if(rc != CMMD_ERR_OK)
		__socket_close(cpal_handle, cpal_key_handle, s);
...
static int socket_update(cpal_handle_t *cpal_handle, cmmd_socket_update_cmd_t *cmd)
{
	struct socket *s;
	unsigned short rc = 0; // rc=2 OK, rc=0 KO, cmm_client error code hack 
...
exit:
	...
	if (del_old_route)	
	{
		...
		__cmmRouteDeregister(cpal_handle, &old_route, "socket");
	}
...
static int socket_close(cpal_handle_t *cpal_handle, cpal_handle_t *cpal_key_handle, cmmd_socket_close_cmd_t *cmd)
{
	struct socket *s;
	int ret = 0;
	...
	ret = __socket_close(cpal_handle, cpal_key_handle, s);
...
int socket_daemon(cpal_handle_t *cpal_handle, cpal_handle_t *cpal_key_handle, int fc, u_int8_t *cmd_buf, u_int16_t cmd_len, u_int16_t *res_buf, u_int16_t *res_len)
{
	int rc = 0, skipcount;
	...
	case CMMD_CMD_SOCKET_OPEN:
	{
		...
		*res_len = 2;
		...
		rc = socket_open(cpal_handle, cpal_key_handle, cmd);
		if (rc >= 0)
		{
			res_buf[0] = rc;
			rc = 0;
	...
	case CMMD_CMD_SOCKET_UPDATE:
	{
		...
		*res_len = 2;
		...
		rc = socket_update(cpal_handle, cmd);
		if (rc >= 0)
		{
			res_buf[0] = rc;
			rc = 0;
	...
	case CMMD_CMD_SOCKET_CLOSE:
	{
		...
		*res_len = 2;
		...
		rc = socket_close(cpal_handle, cpal_key_handle, cmd);
		if (rc >= 0)
		{
			res_buf[0] = rc;
			rc = 0;
