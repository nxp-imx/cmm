...
extern int tunnel_send_cmd(cpal_handle_t *cpal_handle, int request, struct interface *itf);
...
static struct interface_addr *__addr_add(struct interface *itf)
{
	...
	cmm_print(DEBUG_INFO, "%s: success for interface(%d)\n", __func__, itf->ifindex);
...
static struct interface_addr *__addr_find(struct interface *itf, unsigned int *ipaddr, unsigned int len)
{
	...
	cmm_print(DEBUG_ERROR, "%s: error for interface(%d)\n", __func__, itf->ifindex);
	return NULL;
...
static void mr_update(cpal_handle_t *cpal_handle,struct ip6_4rd_map_msg *mr_msg)
{
	struct interface *itf;
	struct map_rule *mr;
	...
	if (!mr)
	{
		...
		if (!(itf->tunnel_flags & TNL_4RD))
		{
			...
			tunnel_send_cmd(cpal_handle, UPDATE,itf);
		}
...
static void mr_remove(cpal_handle_t *cpal_handle,struct ip6_4rd_map_msg *mr_msg)
{
	struct interface *itf;
	struct list_head *entry;
...
static void __itf_remove(struct interface *itf)
{
	...
	if (__itf_is_l2tp(itf))
		l2tp_itf_del(itf_table.cpal_handle, itf);
...
void __itf_update_connection(cpal_handle_t *cpal_handle, int ifindex)
{
	struct list_head *entry, *next_entry;
	struct RtEntry *route;
	int i;

	cmm_print(DEBUG_INFO, "%s: interface(%d) enter\n", __func__, ifindex);
	for (i = 0; i < ROUTE_HASH_TABLE_SIZE * 2; i++)
	{
		for (entry = list_first(&rt_table[i]); next_entry = list_next(entry), entry != &rt_table[i]; entry = next_entry)
		{
			...
			{
				cmm_print(DEBUG_INFO, "%s: (%d:%d - %d:%d) continue\n",
				          __func__, route->oifindex, ifindex, route->phys_oifindex, ifindex);
				continue;
			}
#ifndef VLAN_FILTER
			...
			__cmmRouteUpdate(cpal_handle, route);
		}
	}
}
...
static void __updatelink(struct interface_table *ctx, struct ifinfomsg *ifi, struct rtattr *tb[], int dellink)
{
	...
	if (!itf)
	{
		...
		{
			cmm_print(DEBUG_ERROR, "%s: trying to remove a link on a down interface(%d)\n",
			          __func__, ifi->ifi_flags);
			goto out;
		}
	...
	{
		if (__itf_is_up(itf))
			__tunnel_add(ctx->cpal_handle, itf);
		else if(dellink)
			__tunnel_del(ctx->cpal_handle, ctx->cpal_key_handle, itf);
		else
			__tunnel_update(ctx->cpal_handle, itf);
	}
	else if (__itf_is_l2tp(itf))
	{
		if (__itf_is_up(itf))
			/* L2TP interface being a virtual interface, the physical and logical ifindices are the same */
			l2tp_itf_add(ctx->cpal_handle, ADD, itf);
		else
			__l2tp_itf_del(ctx->cpal_handle, itf);
	}
	else
	{
#ifdef WIFI_ENABLE
		...
		//	cmm_print( DEBUG_INFO, "%s : vap : %s\n", __func__, itf->ifname);
			...
			if (__itf_is_up(itf))
				cmmFeWiFiUpdate(ctx->cpal_handle, ctx->fd, ADD, itf);
			else
				cmmFeWiFiUpdate(ctx->cpal_handle, ctx->fd, REMOVE, itf);
		}
		else
#endif
		if (__itf_is_programmed(itf->phys_ifindex) > 0)
		{
			if (__itf_is_pppoe(itf) && ctx->fp)
			{
				if (__itf_is_up(itf) && (itf->itf_flags & ITF_PPPOE_SESSION_UP))
					cmmFePPPoEUpdate(ctx->cpal_handle, ADD, itf);
				else
					cmmFePPPoEUpdate(ctx->cpal_handle, REMOVE, itf);
			}
			else if (__itf_is_vlan(itf))
			{
				...
				{
					if (__itf_is_up(itf))
						cmmFeVLANUpdate(ctx->cpal_handle, ADD, itf);
					else
						cmmFeVLANUpdate(ctx->cpal_handle, REMOVE, itf);
				}
			}
			else if (__itf_is_macvlan(itf))
			{
				if (__itf_is_up(itf)) {
					cmmFeMacVlanUpdate(ctx->cpal_handle,ctx->fd,ADD,itf);
				}
				else  {
					cmmFeMacVlanUpdate(ctx->cpal_handle,ctx->fd,REMOVE,itf);
				}
			}
		}
		...
		{
#if defined(LS1043)
			cmmFeUpdateAllBridgedIfs(ctx->cpal_handle, ctx->fd, itf);
#else
#ifdef WIFI_ENABLE	
			if (__itf_is_up(itf))	
				cmmFeWiFiBridgeUpdate(ctx->cpal_handle, ctx->fd, ADD, itf);
#endif
#endif
		}
		...
		{
			itf->flags |= FPP_NEEDS_UPDATE;
			cmmFeBridgedIfUpdate(ctx->cpal_handle, ctx->fd, itf);
		}
#endif
	}

	if (dellink && !__itf_is_up(itf) && (ifi->ifi_change == 0xffffffff)) {
		cmm_print(DEBUG_INFO, "%s: delete link(%d)\n", __func__, ifi->ifi_index);
		__itf_remove(itf);
	}
...
static void updatelink(struct interface_table *ctx, struct ifinfomsg *ifi, struct rtattr *tb[], int dellink)
{
	...
	__itf_update_connection(ctx->cpal_handle, ifi->ifi_index);
	mc_update_table(ctx->cpal_handle, tb, ifi);
...
found:
	cmm_print(DEBUG_INFO, "%s: interface(%d) found\n", __func__, ifindex);
	return itf;
}
...
static int __cmmGetLink(struct interface_table *ctx)
{
	...
	{
		cmm_print(DEBUG_ERROR, "%s: cmm_rtnl_dump_request(%d) failed\n", __func__, ifi.ifi_index);
		goto out;
	}
...
int itf_get_ipaddr(int ifindex, int family, unsigned char scope, unsigned int *ipaddr, unsigned int *target)
{
	...
	if (!itf)
	{
		cmm_print(DEBUG_ERROR,"%s: Error - no interface for ifindex(%d)\n", __func__, ifindex);
		goto unlock;
	}
...
int __itf_get_macaddr(struct interface *itf, unsigned char *macaddr)
{
	if (!itf->macaddr_len)
	{
		cmm_print(DEBUG_ERROR, "%s: Error - zero length\n", __func__);
		goto err;
	}
...
int itf_get_macaddr(int ifindex, unsigned char *macaddr)
{
	...
	if (!itf)
	{
		cmm_print(DEBUG_ERROR, "%s: Error - invalid interface(%d)\n", __func__, ifindex);
		goto unlock;
	}
	...
	if (rc < 0)
	{
		cmm_print(DEBUG_ERROR, "%s: __itf_get_macaddr(%d) failed\n", __func__, ifindex);
		goto unlock;
	}
...
int ____itf_get_name(struct interface *itf, char *ifname, int len)
{
	if (!if_indextoname(itf->ifindex, itf->ifname))
	{
		...
		if (itf->ifname[0] == '\0')
		{
			cmm_print(DEBUG_ERROR, "%s: Error ifname NULL\n", __func__);
			goto err;
		}
	}
...
int __itf_get_name(int ifindex, char *ifname, int len)
{
	...
	if (!itf)
	{
		cmm_print(DEBUG_ERROR, "%s: __itf_find(%d) failed\n", __func__, ifindex);
		goto out;
	}
...
#ifndef DPDK_ENABLE
	if (FPP_ERR_OK != (ret = cpal_write(cpal_handle, FPP_CMD_PORT_UPDATE , sizeof(cmd), (unsigned short *) &cmd)))
	{
...
int itf_table_init(struct interface_table *ctx)
{
	...
	if (!ctx->cpal_handle)
	{
		cmm_print(DEBUG_ERROR, "%s::%d: cpal_ff_open() %s\n", __func__, __LINE__, strerror(errno));
		goto err3;
	}
	...
	{
		cmm_print(DEBUG_ERROR, "%s::%d: cpal_key_open() %s\n", __func__, __LINE__, strerror(errno));
		goto err4;
	}
#endif
#endif
	/* either open second connection or use the same one, better open second one */
#ifndef DPDK_ENABLE
#ifdef WIFI_ENABLE
	cmmWiFiReset(ctx->cpal_handle);
#endif
	cmmVlanReset(ctx->cpal_handle);
#endif
...
#ifdef DPDK_ENABLE
		data.l_action = LINK_ADD;
		data.l_portid = ifi->ifi_index;
#endif
	}
	else
	{
#ifdef DPDK_ENABLE
		data.l_action = LINK_DELETE;
		data.l_portid = ifi->ifi_index;
#endif
		cmm_print(DEBUG_INFO, "%s: RTM_DELLINK %s\n", __func__, if_indextoname(ifi->ifi_index, ifname));
	}

	cmm_print(DEBUG_INFO, "LINK: %s: ifinfomsg family: %x, type: %x, index: %d, flags: %x, change: %x\n", __func__,
...
#ifdef DPDK_ENABLE
	if (tb[IFLA_ADDRESS]) {
		memcpy(data.l_mac, RTA_DATA(tb[IFLA_ADDRESS]), sizeof(uint8_t) * MAX_MAC_LENGTH);
	}
	if (tb[IFLA_IFNAME]) {
		memcpy(data.l_name, RTA_DATA(tb[IFLA_IFNAME]), sizeof(uint8_t) * MAX_NAME_LENGTH);
	}
	if(tb[IFLA_OPERSTATE]) {
		data.l_state = *(uint8_t *)tb[IFLA_OPERSTATE];
	}
	data.msg_type = CMM_LINK;
	if (tb[IFLA_MTU]) {
		data.l_mtu = *(int *) RTA_DATA(tb[IFLA_MTU]);
	}
	fpr_send_message(globalConf.ct.unix_sock, &data);
	cmm_print(DEBUG_INFO, "CMM_LINK message sent\n");
#endif
...
#ifdef DPDK_ENABLE
		data.a_action = ADDR_DELETE;
#endif
		deladdr(ctx, ifa, tb);
	}

#ifdef DPDK_ENABLE
	data.a_portid = ifa->ifa_index;
	if (ifa->ifa_family == AF_INET) {
		data.msg_type = CMM_ADDR4;
		memcpy(&data.a_addr, ipaddr, sizeof(struct in_addr));
	} else if (ifa->ifa_family == AF_INET6) {
		data.msg_type = CMM_ADDR6;
		memcpy(&data.a_addr, ipaddr, sizeof(struct in6_addr));
	}
	data.a_prefixlen = ifa->ifa_prefixlen;
	cmm_print(DEBUG_CRIT, "ADDR msg sent\n");
	/* open a differnet connection for it */
	fpr_send_message(globalConf.ct.unix_sock, &data);
#endif
out:
	return RTNL_CB_CONTINUE;
