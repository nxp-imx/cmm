...
#include "module_rx.h"
	...
	/* IPv4 */
	int cmmFeCtUpdate4(cpal_handle_t *cpal_handle, int action, struct ctTable *ctEntry);

	int __cmmFeRouteUpdate(cpal_handle_t* cpal_handle, int action, struct fpp_rt *route);

	/* IPv6 */
	int cmmFeCtUpdate6(cpal_handle_t *cpal_handle, int action, struct ctTable *ctEntry);
	...
	int cmmFeFFControl(cpal_handle_t* cpal_handle, u_int8_t *cmd_buf, u_int16_t cmd_len, u_int16_t *res_buf, u_int16_t *res_len);
	int cmmFeL2FlowUpdate(cpal_handle_t* cpal_handler, int request, struct l2flowTable *l2flow);

static inline int cmmFeRouteUpdate(cpal_handle_t* cpal_handle, int request, struct fpp_rt *fpp_route)
{
	int action;
	...
	return __cmmFeRouteUpdate(cpal_handle, action, fpp_route);
...
static inline int cmmFeCtUpdate(cpal_handle_t *cpal_handle, int request, struct ctTable *ctEntry)
{
	int action;
	...
	if (ctEntry->family == AF_INET)
		return cmmFeCtUpdate4(cpal_handle, action, ctEntry);
	else
		return cmmFeCtUpdate6(cpal_handle, action, ctEntry);
