...
int cmmFeVLANUpdate(cpal_handle_t *cpal_handle, int request, struct interface *itf)
{
...
#ifndef DPDK_ENABLE
		ret = cpal_write(cpal_handle, FPP_CMD_VLAN_ENTRY, sizeof(fpp_vlan_cmd_t), (unsigned short *) &cmd);
	...
	case ACTION_UPDATE:
		...
		ret = cpal_write(cpal_handle, CMD_VLAN_ENTRY, sizeof(struct VlanCmd), (unsigned short *) &cmd);
		if (ret == NO_ERR)
		{
			itf->flags &= ~FPP_NEEDS_UPDATE;
		}
...
#ifndef DPDK_ENABLE
	ret = cpal_write(cpal_handle, FPP_CMD_VLAN_RESET, 0, NULL);
	if (ret == FPP_ERR_OK)
	{
...
int cmmVlanProcessClientCmd(cpal_handle_t *cpal_handle, int function_code, u_int8_t *cmd_buf, u_int16_t cmd_len, u_int16_t *res_buf, u_int16_t *res_len)
{
	...
	case CMMD_ACTION_REGISTER:
		*res_len = 2;
		...
		rc = cmmFeVLANUpdate(cpal_handle, ADD, itf);
		if (rc > 0)
		{
			res_buf[0] = rc;
			rc = 0;
	...
	case CMMD_ACTION_DEREGISTER:
		*res_len = 2;
		...
		rc = cmmFeVLANUpdate(cpal_handle, REMOVE, itf);
		if (rc > 0)
		{
			res_buf[0] = rc;
			rc = 0;
