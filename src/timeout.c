...
int cmmFeGetTimeout(cpal_handle_t *cpal_handle, struct ctTable *ctEntry, unsigned int *timeout)
{
	...
	if (ctEntry->family == AF_INET)
	{
		...
		rc = cpal_query(cpal_handle, FPP_CMD_IPV4_GET_TIMEOUT, sizeof(fpp_ct_cmd_t), (unsigned short *)&cmd, &rcvBytes, rcvBuffer);
	}
	else
	{
		...
		rc = cpal_query(cpal_handle, FPP_CMD_IPV6_GET_TIMEOUT, sizeof(fpp_ct6_cmd_t), (unsigned short *)&cmd6, &rcvBytes, rcvBuffer);
	}
