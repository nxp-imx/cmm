...
extern  void __itf_update_connection(cpal_handle_t *cpal_handle, int ifindex);
 ...
 ************************************************************/
int tunnel_send_cmd(cpal_handle_t *cpal_handle, int request, struct interface *itf)
{
...
#ifndef DPDK_ENABLE
		ret = cpal_write(cpal_handle, FPP_CMD_TUNNEL_ADD, sizeof(fpp_tunnel_create_cmd_t), (unsigned short *)&cmd);
	...
	else
	{
		...
		ret = cpal_write(cpal_handle, FPP_CMD_TUNNEL_UPDATE, sizeof(fpp_tunnel_create_cmd_t), (unsigned short *)&cmd);
		if (ret == FPP_ERR_OK)
		{
			itf->flags &= ~FPP_NEEDS_UPDATE;
		}
...
static int tunnel_send_del(cpal_handle_t *cpal_handle, struct interface *itf)
{
...
#ifndef DPDK_ENABLE
	ret = cpal_write(cpal_handle, FPP_CMD_TUNNEL_DEL, sizeof(fpp_tunnel_del_cmd_t), (unsigned short *) &cmd);
...
void __tunnel_remove_flow(cpal_handle_t *cpal_key_handle, struct interface *itf)
{
	if (itf->flow_orig)
		if (!cmmFlowKeyEngineRemove(cpal_key_handle, itf->flow_orig))
		{
	...
	if (itf->flow_rep)
		if (!cmmFlowKeyEngineRemove(cpal_key_handle, itf->flow_rep))
		{
 ...
 ************************************************************/
static int tunnel_update_sa(cpal_handle_t *cpal_handle, struct interface *itf, unsigned char orig)
{
...
#ifndef DPDK_ENABLE
	if ((ret = cpal_write(cpal_handle, FPP_CMD_TUNNEL_SEC, sizeof(fpp_tunnel_sec_cmd_t), (unsigned short *) &cmd)))
	{
 ...
 * 
 ************************************************************/
int __tunnel_add(cpal_handle_t *cpal_handle, struct interface *itf)
{
 ...
 */
		...
		cmmFeRouteUpdate(cpal_handle, ADD | UPDATE, itf->rt.fpp_route);
	}
	else
		enabled = 1;
...
program:
	...
	rc = tunnel_send_cmd(cpal_handle, ADD | UPDATE, itf);
	...
	{
		if (itf->flow_orig)
			tunnel_update_sa(cpal_handle, itf, 1);

		if (itf->flow_rep)
			tunnel_update_sa(cpal_handle, itf, 0);
	}
err:
	return rc;
}
 ...
 *
 ************************************************************/
static int tunnel_add(cpal_handle_t *cpal_handle, cpal_handle_t *cpal_key_handle, char *name, unsigned char ipsec, char tnl_type, u_int16_t *res_buf, u_int16_t *res_len)
{
	int ifindex;
	...
	rc  = __tunnel_add(cpal_handle, itf);
	if (rc >= 0)
	{
		res_buf[0] = rc;
		rc = 0;
	...
	if(update_tnl_flows)
		__tunnel_remove_flow(cpal_key_handle, itf);

        if(update_connections)
               __itf_update_connection(cpal_handle, itf->ifindex);
 ...
 *
 ************************************************************/
int __tunnel_del(cpal_handle_t *cpal_handle, cpal_handle_t *cpal_key_handle, struct interface *itf)
{
	int rc = tunnel_send_del(cpal_handle, itf);


	__cmmRouteDeregister(cpal_handle, &itf->rt, "tunnel");
 ...
 *
 ************************************************************/
static int tunnel_del(cpal_handle_t *cpal_handle, cpal_handle_t *cpal_key_handle, char *name, u_int16_t *res_buf, u_int16_t *res_len)
{
	int ifindex;
	struct interface *itf;
	int rc = 0;
	...
	*res_len = 2;
	...
	itf->tunnel_flags &= ~TNL_4RD;
	#endif
	rc = __tunnel_del(cpal_handle, cpal_key_handle, itf);
	if (rc >= 0)
	{
		res_buf[0] = rc;
		rc = 0;
 ...
 *
 ************************************************************/
int __tunnel_update(cpal_handle_t *cpal_handle, struct interface *itf)
{
	int rc = 0;
	itf->flags |= FPP_NEEDS_UPDATE;
	itf->tunnel_enabled = 0;
	rc = tunnel_send_cmd(cpal_handle, UPDATE, itf);
	return rc;
}
...
int tunnel_conv_id_set(cpal_handle_t *cpal_handle, char *name,  char *buffer, int buffer_size)
{
       int ifindex;
       struct interface *itf;
       int rc = 0;
...
#ifndef DPDK_ENABLE
       rc = cpal_write(cpal_handle, FPP_CMD_TUNNEL_4rd_ID_CONV_psid, sizeof(fpp_tunnel_id_conv_cmd_t),(unsigned short *)buffer);
       if(rc != 0)
 ...
 ************************************************************/
int tunnel_daemon_msg_recv(cpal_handle_t *cpal_handle, cpal_handle_t *cpal_key_handle, int function_code, u_int8_t *cmd_buf, u_int16_t cmd_len, u_int16_t *res_buf, u_int16_t *res_len)
{
	...
	case CMMD_CMD_TUNNEL_ADD:
		return tunnel_add(cpal_handle, cpal_key_handle, tnl->name, tnl->ipsec, tnl->tunnel_type, res_buf, res_len);

	case CMMD_CMD_TUNNEL_DEL:
		return tunnel_del(cpal_handle, cpal_key_handle, tnl->name, res_buf, res_len);

	case CMMD_CMD_TUNNEL_SHOW:
		return tunnel_show(cpal_handle, tnl->name, res_buf, res_len);
#ifdef SAM_LEGACY
        ...
        case CMMD_CMD_TUNNEL_IDCONV_psid:
	{
		...
		*res_len = 2;
               ...
               return tunnel_conv_id_set(cpal_handle, (char*)tnl_IdConv->name,(char*)cmd_buf, sizeof(fpp_tunnel_id_conv_cmd_t));
	}
#endif
	default:
 ...
 *
 ************************************************************/
void __cmmTunnelUpdateWithRoute(cpal_handle_t *cpal_handle, struct RtEntry *route)
{
	struct interface *itf;
	...
	for (i = 0; i < ITF_HASH_TABLE_SIZE; i++)
	{
		for (entry = list_first(&itf_table.hash[i]); entry != &itf_table.hash[i]; entry = list_next(entry))
		{
			...
			if (itf->rt.route == route)
			{
				...
				__tunnel_add(cpal_handle, itf);

				__cmmFPPRouteDeregister(cpal_handle, fpp_route, "tunnel");
			}
		}
	}
}
