...
*
******************************************************************/
int cmmFePPPoEUpdate(cpal_handle_t *cpal_handle, int request, struct interface *itf)
{
	fpp_pppoe_cmd_t cmd;
	short ret;
	int action;
...
#ifndef DPDK_ENABLE
		ret = cpal_write(cpal_handle, FPP_CMD_PPPOE_ENTRY, sizeof(fpp_pppoe_cmd_t), (unsigned short *) &cmd);
	...
	case ACTION_UPDATE:
		...
		ret = cpal_write(cpal_handle, CMD_PPPOE_ENTRY, sizeof(struct PPPoECommand), (unsigned short *) &cmd);
		if (ret == FPP_ERR_OK)
		{
			itf->flags &= ~FPP_NEEDS_UPDATE;
		}
...
int cmmPPPoEAutoGetIdle( struct interface* itf , unsigned long* rcv_sec , unsigned long* xmit_sec)
{
        ...
        ret = cpal_query(itf_table.cpal_handle, FPP_CMD_PPPOE_GET_IDLE, sizeof(fpp_pppoe_idle_t), (unsigned short *) &cmd, &rcvlen, (unsigned short*) &rcvbuf[0]);
