...
int __cmm_l2flow_deregister(cpal_handle_t* cpal_handler, struct l2flow *l2flow_tmp)
{
	...
	if(!l2flow_entry){
		...
		if(!cmmFeL2FlowUpdate(cpal_handler, REMOVE, l2flow_entry))
...
int __cmm_l2flow_register(cpal_handle_t* cpal_handler, char action, struct l2flow *l2flow_tmp, int iifi_idx, int oifi_idx, int flags, short mark)
{
	...
	if(allowed){
		if(!cmmFeL2FlowUpdate(cpal_handler, ADD|UPDATE, entry))
		...
		cmmFeL2FlowUpdate(cpal_handler, REMOVE, entry);
...
int __cmm_l2flow_reset(cpal_handle_t* cpal_handler)
{
	int i;
...
#ifndef DPDK_ENABLE
	if (cpal_write(cpal_handler, FPP_CMD_RX_L2BRIDGE_FLOW_RESET, 0, NULL))
	{
...
int cmm_l2flow_netlink_rcv(const struct sockaddr_nl *who, struct nlmsghdr *nlh, void *arg)
{
	...
	if (nlh->nlmsg_type == L2FLOW_MSG_ENTRY)
	{
		...
		if((action == L2FLOW_ENTRY_NEW) || (action == L2FLOW_ENTRY_UPDATE))
		{
			...
			__cmm_l2flow_register(ctx->cpal_handle, action, &l2flow_tmp, iifi_idx, oifi_idx, l2msg->flags, mark);
	...
	{
		__pthread_mutex_lock(&brMutex);
		__cmm_l2flow_reset(ctx->cpal_handle);
		__pthread_mutex_unlock(&brMutex);
	}
out:
...
#ifndef DPDK_ENABLE
	if (cpal_write(cpal_handle, FPP_CMD_BRIDGED_ITF_UPDATE, sizeof(br_cmd), (unsigned short *)&br_cmd)){
...
*/
int cmmFeUpdateAllBridgedIfs( cpal_handle_t *cpal_handle, int fd, struct interface *bitf)
{
	struct list_head *entry;
	struct interface *itf;
	short ret = 0;
		...
		for (entry = list_first(&itf_table.hash[i]); entry != &itf_table.hash[i]; entry = list_next(entry))
		{
			...
			if(j < MAX_PORTS)
			{
				...
				if ((____itf_is_programmed(itf)) && __itf_is_up(itf) )
				{
					...
					ret = cmmFeBridgedIfUpdate(cpal_handle, fd, itf);
				}
#ifndef VLAN_FILTER
			}
#endif
...
#ifndef DPDK_ENABLE
	if (cpal_write(ctx->cpal_handle, FPP_CMD_RX_L2BRIDGE_MODE, sizeof(br_cmd), (unsigned short *)&br_cmd)){
 ...
 *
 ********************************************************************************/
int cmmL2BridgeProcessClientCmd(cpal_handle_t* cpal_handle, int fc, u_int8_t *cmd_buf, u_int16_t cmd_len, u_int16_t *res_buf, u_int16_t *res_len)
{
	fpp_l2_bridge_enable_cmd_t *bridgeEnableCmd;
	fpp_l2_bridge_add_entry_cmd_t *bridgeAddCmd;
	fpp_l2_bridge_remove_entry_cmd_t *bridgeRemoveCmd;
...
CPAL_CMD:
	return cpal_cmd(cpal_handle, fc, (u_int16_t*)cmd_buf, cmd_len, res_buf, res_len);
