...
static inline int cmmFcIsConntrackAllowed(cpal_handle_t *cpal_handle, struct nf_conntrack *ct, struct RtEntry **rtEntryOrig)
{
	if (nfct_get_attr_u8(ct, ATTR_ORIG_L3PROTO) == AF_INET)
		return cmmFcIsConntrack4Allowed(cpal_handle, ct, rtEntryOrig);
	else
		return cmmFcIsConntrack6Allowed(cpal_handle, ct, rtEntryOrig);
}
