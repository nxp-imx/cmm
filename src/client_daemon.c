...
int cmmDaemonInit(struct cmm_daemon *ctx)
{
	...
	if (!ctx->cpal_handle)
	{
		cmm_print(DEBUG_CRIT, "%s: cpal_ff_open() failed, %s\n", __func__, strerror(errno));
		goto err2;
	}
...
#endif
#endif
	voice_buffer_reset(ctx->cpal_handle);
...
static int cmmCommandParse(struct cmm_daemon *ctx, int function_code, u_int8_t *cmd_buf, u_int16_t cmd_len, u_int16_t *res_buf, u_int16_t *res_len)
{
	...
	switch (function_code)
	{
		...
		return cmmCtHandle(ctx->cpal_handle, function_code, cmd_buf, cmd_len, res_buf, res_len);

	case CMMD_CMD_IPV4_FF_CONTROL:
		return cmmFeFFControl(ctx->cpal_handle, cmd_buf, cmd_len, res_buf, res_len);
	...
	case CMMD_CMD_MC6_MODE:
		return cmmMc6ProcessClientCmd(ctx->cpal_handle, function_code, cmd_buf, cmd_len, res_buf, res_len);

	case CMMD_CMD_MC4_MULTICAST:
	case CMMD_CMD_MC4_RESET:
		return cmmMc4ProcessClientCmd(ctx->cpal_handle, function_code, cmd_buf, cmd_len, res_buf, res_len);
...
#endif
		return tunnel_daemon_msg_recv(ctx->cpal_handle, ctx->cpal_key_handle, function_code, cmd_buf, cmd_len, res_buf, res_len);

        case CMMD_CMD_PPPOE_RELAY_ADD:
        case CMMD_CMD_PPPOE_RELAY_REMOVE:
		return cmmRelayProcessClientCmd(ctx->cpal_handle, function_code, cmd_buf, cmd_len, res_buf, res_len);

	case CMMD_CMD_VLAN_ENTRY:
		return cmmVlanProcessClientCmd(ctx->cpal_handle, function_code, cmd_buf, cmd_len, res_buf, res_len);
#if defined (LS1043)
	case CMMD_CMD_IPR_V4_STATS:
	case CMMD_CMD_IPR_V6_STATS:
		return cmmIprStatsProcessClientCmd(ctx->cpal_handle, function_code, cmd_buf, cmd_len, res_buf, res_len);
#endif
	case CMMD_CMD_EXTROUTE:
		return cmmRouteProcessClientCmd(ctx->cpal_handle, function_code, cmd_buf, res_buf, res_len);
	...
	case CMMD_CMD_SOCKET_SHOW:
		return socket_daemon(ctx->cpal_handle, ctx->cpal_key_handle, function_code, cmd_buf, cmd_len, res_buf, res_len);

	case CMMD_CMD_VOICE_FILE_LOAD:
		return voice_file_load(ctx->cpal_handle, (cmmd_voice_file_load_cmd_t *)cmd_buf, res_buf, res_len);

	case CMMD_CMD_VOICE_FILE_UNLOAD:
		return voice_file_unload(ctx->cpal_handle, (cmmd_voice_file_unload_cmd_t *)cmd_buf, res_buf, res_len);

	case CMMD_CMD_L2TP_SESSION_CREATE:
	case CMMD_CMD_L2TP_SESSION_DESTROY:
		return l2tp_daemon(ctx->cpal_handle,function_code, (cmmd_l2tp_session_t *) cmd_buf, cmd_len, res_buf, res_len);
	...
	case FPP_CMD_RX_L2BRIDGE_QUERY_ENTRY:
		return cmmL2BridgeProcessClientCmd(ctx->cpal_handle, function_code, cmd_buf, cmd_len, res_buf, res_len);

	case FPP_CMD_NATPT_OPEN:
		return cmmNATPTOpenProcessClientCmd(ctx->cpal_handle, cmd_buf, cmd_len, res_buf, res_len);
...
CPAL_CMD:
	...
	return cpal_cmd(ctx->cpal_handle, function_code, (unsigned short *)cmd_buf, cmd_len, (unsigned short *)res_buf, res_len);
}
