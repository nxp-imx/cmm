 ...
 *
 ************************************************************/
int mc6_send_command( cpal_handle_t *cpal, unsigned short action, struct mcast_entry *mc , char * Ifname )
{
	int ret = 0, i,j;
...
#ifndef DPDK_ENABLE
	ret = cpal_write(cpal, CMMD_CMD_MC6_MULTICAST,
 ...
 *
 ***********************************************************/
int mc4_send_command( cpal_handle_t *cpal, unsigned short action, struct mcast_entry *mc, char * Ifname)
{
	int ret = 0, i, j;
	char sndBuffer[256], program = 0;
	...
	entryCmd->num_output = j;
#ifndef DPDK_ENABLE
	ret = cpal_write(cpal, CMMD_CMD_MC4_MULTICAST,
 ...
 *
 ************************************************************/ 
void mc_update_table( cpal_handle_t *cpal, struct rtattr *tb[], struct ifinfomsg *ifi )
{
	struct mcast_entry *mc = NULL;
	struct list_head *entry;
			...
			for(jj =0 ; jj < mc->num_output; jj++)
			{
				...
				if(! strncmp(mc->listener[jj].output_device_str, ifname, sizeof(mc->listener[jj].output_device_str) ) )
				{
					if( mc->family == AF_INET )
						mc4_send_command( cpal, action, mc, ifname);
					else
						mc6_send_command( cpal, action, mc, ifname);
				}
			}
