...
#ifndef LS1043
static int cmmFeWiFiGetBridge( cpal_handle_t *cpal_handle, int fd, struct interface *witf );
#endif
...
#ifndef LS1043
		cmmFeWiFiGetBridge(cpal_handle, fd, itf);
#endif
...
#ifndef DPDK_ENABLE
	ret = cpal_write(cpal_handle, FPP_CMD_WIFI_VAP_ENTRY,
		...
		for (entry = list_first(&itf_table.hash[i]); entry != &itf_table.hash[i]; entry = list_next(entry))
		{
			...
			if( (j < MAX_PORTS) && (itf->flags & FPP_PROGRAMMED) && __itf_is_up(itf) )
			{
						...
						(itf->macaddr[5] ^  bitf->macaddr[5]) )
				{
					...
					ret = cmmFeWiFiUpdate(cpal_handle, fd, UPDATE, itf);
...
#ifndef DPDK_ENABLE
	ret = cpal_write(cpal_handle, FPP_CMD_WIFI_VAP_RESET, 0, NULL);
	if (ret == FPP_ERR_OK)
	{
		if (!cmmResetVWD()) {
