...
static inline void
__jhash_2hashes(const void *key, u_int32_t length, u_int32_t *pc,
		u_int32_t *pb, unsigned check_align)
{
	u_int32_t a, b, c;

	/* Set up the internal state */
	a = b = c = JHASH_GOLDEN_RATIO + ((u_int32_t)length) + *pc;
	c += *pb;
	 ...
	 */
	const u_int32_t *k = (u_int32_t *)((uintptr_t)key & (uintptr_t)~3);
	const u_int32_t s = ((uintptr_t)key & 3) * CHAR_BIT;
	if (!check_align || s == 0) {
		while (length > 12) {
			a += k[0];
			b += k[1];
 ...
 */
static inline void
jhash_2hashes(const void *key, u_int32_t length, u_int32_t *pc, u_int32_t *pb)
{
	__jhash_2hashes(key, length, pc, pb, 1);
}

/**
 * Same as jhash_32b, but takes two seeds and return two u_int32_ts.
 * pc and pb must be non-null, and *pc and *pb must both be initialized
 * with seeds. If you pass in (*pb)=0, the output (*pc) will be
 * the same as the return value from jhash_32b.
 *
 * @param k
 *   Key to calculate hash of.
 * @param length
 ...
 */
static inline void
jhash_32b_2hashes(const u_int32_t *k, u_int32_t length, u_int32_t *pc, u_int32_t *pb)
{
	__jhash_2hashes((const void *) k, (length << 2), pc, pb, 0);
}
...
{
	u_int32_t initval2 = 0;

	jhash_2hashes(key, length, &initval, &initval2);
 ...
 */
static inline u_int32_t
jhash_32b(const u_int32_t *k, u_int32_t length, u_int32_t initval)
{
	u_int32_t initval2 = 0;

	jhash_32b_2hashes(k, length, &initval, &initval2);

	return initval;
}

static inline u_int32_t
__jhash_3words(u_int32_t a, u_int32_t b, u_int32_t c, u_int32_t initval)
{
...
{
	return __jhash_3words(a + 12, b + 12, c + 12, initval);
}
...
{
	return __jhash_3words(a + 8, b + 8, 8, initval);
}
...
{
	return __jhash_3words(a + 4, 4, 4, initval);
}
