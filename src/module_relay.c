...
static int cmmRelayAdd(cpal_handle_t * cpal_handler, struct fpp_relay_info *sh, u_int16_t *res_buf, u_int16_t *res_len)
{
   ...
   * names for bridge interfaces */
    ...
    cmm_print(DEBUG_INFO,
              ...
              cmd->relaysesID);
#endif
    ret = cpal_cmd(cpal_handler, FPP_CMD_PPPOE_RELAY_ENTRY, (unsigned short *) cmd, sizeof(*cmd), res_buf, res_len);
...
#endif
static int cmmRelayRemove(cpal_handle_t * cpal_handler,fpp_relay_info_t *sh, u_int16_t *res_buf, u_int16_t *res_len);

int cmmRelayProcessClientCmd(cpal_handle_t * cpal_handle, int function_code,
                             u_int8_t *cmd_buf, u_int16_t cmd_len, u_int16_t *res_buf, u_int16_t *res_len)
{
    ...
    case CMMD_CMD_PPPOE_RELAY_ADD:
        ...
        cmm_print(DEBUG_INFO,
                  ...
                  sh->relaysesID);
#endif
        rc = cmmRelayAdd(cpal_handle, sh, res_buf, res_len);
        break;

    case CMMD_CMD_PPPOE_RELAY_REMOVE:
        rc = cmmRelayRemove(cpal_handle, sh, res_buf, res_len);
        break;
...
static int cmmRelayRemove(cpal_handle_t * cpal_handler,fpp_relay_info_t *sh, u_int16_t *res_buf, u_int16_t *res_len)
{
    ...
    *res_len = 2;
    ...
    ret = cpal_cmd(cpal_handler, FPP_CMD_PPPOE_RELAY_ENTRY, (unsigned short *) cmd, sizeof(fpp_pppoe_relay_cmd_t), res_buf, res_len);
